...
import { sanitizeString, sanitizeSlug, validateInt, parseIntSafe } from '@/lib/validate';
...
export async function PUT(req, { params }) {
  try {
    ...
    if (shortDescription && shortDescription.length > 500) {
      return errorResponse('Short description must be less than 500 characters', 400);
    }

    // 9. Validate full description
    if (fullDescription && fullDescription.length > 10000) {
      return errorResponse('Full description must be less than 10,000 characters', 400);
    }
    ...
    if (body.categories !== undefined) {
      ...
      categories = body.categories
        .filter(c => c && typeof c === 'object' && c.category && c.subCategory)
        .map(c => ({
      ...
      if (categories.length !== body.categories.length) {
        return errorResponse('Some categories have invalid format', 400);
      }
    } else {
    ...
    if (body.images !== undefined) {
      ...
      const invalidImages = body.images.filter(img => 
        typeof img !== 'string' || (!img.startsWith('/uploads/') && !img.startsWith('http://') && !img.startsWith('https://'))
      );

      if (invalidImages.length > 0) {
        return errorResponse('All gallery images must be valid upload paths or URLs', 400);
      }
    ...
    if (duplicateSlug) {
      return errorResponse('A portfolio item with this slug already exists', 409);
    }
    ...
    if (Array.isArray(existingPortfolio.images)) {
      const removedImages = existingPortfolio.images.filter(img => !images.includes(img));
    ...
    if (err.code === 'P2002') {
      ...
      return errorResponse(`A portfolio item with this ${field} already exists`, 409);
    }
    if (err.code === 'P2025') {
      return errorResponse('Portfolio item not found', 404);
    }
...
export async function DELETE(req, { params }) {
  try {
    ...
    if (err.code === 'P2003') {
      return errorResponse('Cannot delete portfolio item: it is referenced by other records', 400);
    }
    if (err.code === 'P2021') {
      return errorResponse('Database table does not exist', 500);
    }
