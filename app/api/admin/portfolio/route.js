  ...
  try {
    ...
    if (body.categories !== undefined) {
      ...
      if (categories.length !== body.categories.length) {
        return errorResponse('Some categories have invalid format (must have category and subCategory)', 400);
      }
      ...
      for (const cat of categories) {
        ...
        if (cat.subCategory.length < 2) {
          return errorResponse('Sub-category name must be at least 2 characters', 400);
        }
      }
    } else {
    ...
    if (body.features !== undefined) {
      ...
      for (const feature of features) {
        if (feature.length < 2) {
          return errorResponse('Each feature must be at least 2 characters', 400);
        }
      }
    }
