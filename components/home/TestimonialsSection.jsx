...
function VideoTestimonialCard({ testimonial, isMobile }) {
  ...
  // Mobile: tap to go to testimonials page
  ...
  return (
    <>
      ...
      >
        {/* Thumbnail - shown on desktop when not hovering, or as fallback */}
        <img
          src={testimonial.thumbnailUrl}
          alt=""
          loading="eager"
          className={`absolute inset-0 w-full h-full object-cover transition-all duration-500 ${
            (isHovering && !isMobile) || (isMobile && isMobilePlaying) 
              ? "opacity-0 scale-105" 
        ...
        >
          <div className="relative">
            {/* Ping animation - desktop only when not playing */}
            {!isMobile && (
              <div
            ...
            <div className="relative w-12 h-12 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center shadow-lg">
              <div className="flex flex-col items-center">
                {isMobile ? (
                  // Mobile: Show "Watch More" or link indicator
                  <>
                    <svg className="w-4 h-4 text-[#5b8def]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
                    </svg>
                    <span className="text-[7px] font-medium text-[#5b8def] mt-0.5">More</span>
                  </>
                ) : (
        ...
        {/* Mobile: "Tap to see more" indicator at bottom */}
        {isMobile && (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          {/* RIGHT: Card + navigation - PAUSE ON HOVER (desktop only) */}
          <div
              ...
              >
                {hasVideo ? (
                  <div className="h-full flex flex-col">
                    <VideoTestimonialCard testimonial={current} isMobile={isMobile} />
                    <div
                      className="flex flex-col items-center justify-center px-5 pb-5"
                      style={{ height: `${CONTENT_HEIGHT}px` }}
                    >
                      <p className="text-white/90 text-sm leading-relaxed text-center mb-3 line-clamp-2 max-w-lg">
                        "{displayText}"
                      </p>
                      ...
                      {testimonials.length > 1 && (
                        <div className="mt-3 w-full h-0.5 bg-white/10 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-gradient-to-r from-[#5b8def] to-[#20427f] transition-none"
                  ...
                  >
                    ...
                    <p className="text-white/90 text-base leading-relaxed text-center mb-5 line-clamp-3 max-w-lg">
                      "{displayText}"
                    </p>
                  ...
                  {testimonials.map((_, i) => (
                    <button
                      ...
                      className={`h-1.5 rounded-full transition-all ${
                        i === activeIndex ? "bg-[#5b8def] w-6" : "bg-white/20 w-1.5 hover:bg-white/40"
                      }`}
