...
export default function EditTeamMemberPage() {
  ...
  const fetchMember = useCallback(async () => {
    try {
      ...
      if (res.status === 404) {
        setNotFound(true);
        toastService.error("Team member not found");
        return;
      }
      ...
      if (teamMember.image) {
        setImagePreview({
          preview: teamMember.image,
          name: teamMember.image.split("/").pop(),
          isExisting: true,
        });
      }
      ...
      toastService.error(err.message || "Failed to load team member");
    } finally {
      setIsLoading(false);
    }
  }, [params.id]);
  ...
  const handleImageUpload = (e) => {
    ...
    // Revoke previous preview if it's a blob URL
    if (imagePreview?.preview && !imagePreview?.isExisting) {
      URL.revokeObjectURL(imagePreview.preview);
    }
  ...
  const removeImage = () => {
    // Only revoke if it's a blob URL (not existing server image)
    if (imagePreview?.preview && !imagePreview?.isExisting) {
      URL.revokeObjectURL(imagePreview.preview);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const updatePromise = (async () => {
      ...
      if (imagePreview?.file && !imagePreview.isExisting) {
        ...
        // Keep existing image
        uploadedImage = imagePreview.preview;
      }
      // else: imagePreview is null → image will be set to null
      ...
      <AdminLayoutNew>
        <div className="min-h-screen flex items-center justify-center">
          <div className="inline-flex items-center gap-4">
            ...
            <p className="text-slate-500 font-medium">Loading team member...</p>
          </div>
        </div>
      </AdminLayoutNew>
    );
      ...
      <div className="min-h-screen mt-0">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-600 mt-1">Update team member details</p>
          </div>
          <Link href="/admin/team">
            <button
              type="button"
