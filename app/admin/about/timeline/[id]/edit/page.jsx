// app/admin/about/timeline/[id]/edit/page.jsx
'use client';
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-white rounded-2xl border-2 border-slate-200 p-6">
              <div className="w-40 h-5 bg-slate-200 rounded animate-pulse mb-6" />
...
export default function EditTimelinePage() {
  ...
  const fetchItem = async () => {
    try {
      const res = await fetch(`/api/admin/about/timeline/${params.id}`);
      if (res.ok) {
        ...
        customToast.error('Timeline item not found');
        router.push('/admin/about/timeline');
      }
    } catch (error) {
      console.error('Error fetching item:', error);
      customToast.error('Failed to load timeline item');
    } finally {
      setLoading(false);
    }
  };
  ...
  const validateForm = () => {
    ...
    } else if (!/^\d{4}$/.test(formData.year.toString())) {
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-3">
                    <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">Edit Milestone</h1>
                    {hasChanges() && (
                  ...
                  <p className="text-slate-600 mt-1">Update the details of this milestone</p>
                </div>
              </div>
            </div>
            ...
            <Link
              href="/about"
              target="_blank"
              className="inline-flex items-center gap-2 px-4 py-2.5 border-2 border-slate-300 text-slate-700 rounded-xl hover:bg-slate-50 font-semibold transition-all text-sm"
            >
            ...
            <div className="bg-white rounded-2xl border-2 border-slate-200 shadow-lg overflow-hidden">
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                  <FormInput label="Year" required error={errors.year}>
                  ...
                  <FormInput label="Position" hint="Display order" error={errors.position}>
                    <div className="relative">
                      <Icons.Hash className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                      <input
                        type="number"
                        name="position"
                        ...
                        onChange={handleChange}
                        min="1"
                        className="w-full pl-10 pr-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium"
                      />
                    </div>
                  </FormInput>
                </div>
                ...
                <label className="flex items-start gap-4 cursor-pointer group">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-500 mt-0.5">
                      When enabled, this milestone will be visible on your About page.
                    </p>
                  </div>
                </label>
              </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white rounded-2xl border-2 border-slate-200 p-5 shadow-lg">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between py-2 border-b border-slate-100">
                    ...
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${
                      ...
                      <span className={`w-1.5 h-1.5 rounded-full ${
                        formData.published ? 'bg-emerald-500' : 'bg-amber-500'
                      }`} />
                  ...
                  <div className="flex items-center justify-between py-2 border-b border-slate-100">
                    ...
                    <span className="text-sm font-bold text-slate-900">#{formData.position}</span>
                  </div>
                  <div className="flex items-center justify-between py-2">
                    <span className="text-sm text-slate-500 font-medium">Year</span>
                    <span className="text-sm font-bold text-slate-900">{formData.year}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-50 rounded-2xl border-2 border-slate-200 p-5">
                <h4 className="font-semibold text-slate-700 mb-3 text-sm">Quick Actions</h4>
                <div className="space-y-2">
                  <Link
                    href="/about"
                    target="_blank"
                    className="flex items-center gap-2 w-full px-3 py-2.5 text-sm text-slate-600 hover:text-indigo-600 hover:bg-white rounded-xl transition-all font-medium"
                  >
                    <Icons.ExternalLink className="w-4 h-4" />
                    View on Website
                  </Link>
